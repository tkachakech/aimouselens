...
class SettingsWindow:
    ...
    def _build(self):
        ...
        sub("Just paste your key. We know the URLs and Models.")
        ...
        for name in SMART_PROVIDERS.keys():
            ...
            ctk.CTkLabel(row, text=name, width=120, anchor="w", font=ctk.CTkFont(FN, 11, "bold")).pack(side="left")
            
            key_var = tk.StringVar()
            self.smart_entries[name] = key_var
            ctk.CTkEntry(row, textvariable=key_var, placeholder_text="Paste Key...", show="•", height=24).pack(side="left", fill="x", expand=True, padx=5)
            
            ctk.CTkButton(row, text="+ Add", width=50, height=24, fg_color="#10B981", hover_color="#059669", 
                          command=lambda n=name, v=key_var: self._quick_add(n, v.get())).pack(side="right")
