"""
╔══════════════════════════════════════════════════════════════╗
║       AI MOUSE-LENS · ELITE DESIGNER EDITION (v4.0)        ║
║  Luxury HUD · Voice Gallery · Persistence · 503 Retry       ║
╚══════════════════════════════════════════════════════════════╝

Install deps:
    pip install customtkinter pillow pyautogui keyboard requests 
    speechrecognition edge-tts openai-whisper pyaudio pygame pyperclip
...
# Font selection – try premium fonts, fallback to system sans‑serif
def _preferred_font() -> str:
    # check for common "expensive" fonts
    for name in ("Inter", "Segoe UI Variable", "Segoe UI", "Helvetica Neue",
                 "Helvetica", "Ubuntu"):
        try:
            tk.Label(highlightthickness=0, text="test", font=(name, 11))
            return name
        except Exception:
            continue
    return "sans-serif"
...
PROMPT = (
    ...
    "The image shows a screenshot region under the cursor. "
...
VOICE_PROMPT = (
    ...
    "Answer the user's question in 1‑2 concise sentences. "
...
SAFE_MODELS = [
    ...
    "grok-1", "deepseek-chat", "deepseek-vision",
...
# ─────────────────────────────────────────────────────────────────────────────
#  AI ENGINE – with silent 503 retry
# ─────────────────────────────────────────────────────────────────────────────
class AIEngine:
    ...
    @staticmethod
    def _retry_request(method: str, url: str, **kwargs) -> _req.Response:
        """Retry on 503/429 up to 2 times with 1.5s delay, silently."""
        retries = 2
        last_exc = None
    ...
    def _gemini_img(self, b64: str, p: dict) -> str:
        ...
        r = self._retry_request("POST", url, json=body, timeout=30)
    ...
    def _openai_img(self, b64: str, p: dict, base_url: str = "") -> str:
        ...
        r = self._retry_request("POST", url,
    ...
    def _anthropic_img(self, b64: str, p: dict) -> str:
        ...
        r = self._retry_request("POST", "https://api.anthropic.com/v1/messages",
    ...
    def _ollama_txt(self, q: str, p: dict) -> str:
        model = p.get("model", "moondream")
        r = _req.post("http://localhost:11434/api/generate",
                      json={"model": model, "prompt": VOICE_PROMPT + "\nUser: " + q,
...
# ─────────────────────────────────────────────────────────────────────────────
#  FETCH MODELS (unchanged, just a light wrapper)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
#  OVERLAY  (lens ring + Lensy face + result bubble with 📌 pin)
# ─────────────────────────────────────────────────────────────────────────────
class Overlay:
    def __init__(self, root: tk.Misc, cfg: dict, on_bubble_result: Optional[Callable] = None):
        self.root = root
        self.cfg  = cfg
        self._state = "idle"
        self._mx = self._my = 0
        ...
        self.on_result_bubble = on_bubble_result  # callback to notify when bubble text changes (for history)
        self._build_lens()
        self._build_face()
        self._build_bubble()
        self._track()
    ...
    def show_result(self, text: str, status: str):
        ...
        self._bw.deiconify(); self._bw.lift()
        self._place_bubble()
        # restart auto-hide unless pinned
        if self._hide_job:
            self.root.after_cancel(self._hide_job)
        if not self._pinned:
            self._hide_job = self.root.after(AUTO_HIDE_SEC * 1000, self.hide_bubble)
        # Notify that bubble content changed (for history tracking)
        if self.on_result_bubble:
            self.on_result_bubble(text, status)
...
# ─────────────────────────────────────────────────────────────────────────────
#  AUDIO PLAYER (silent background, no overlap)
# ─────────────────────────────────────────────────────────────────────────────
class AudioPlayer:
    ...
    def _speak_bg(self, text: str, on_done):
        ...
        try:
            ...
            if HAS_PYGAME:
                with self._lock:
                    _pygame.mixer.music.stop()       # stop any previous playback
...
# ─────────────────────────────────────────────────────────────────────────────
#  SETTINGS WINDOW – Voice Gallery, history click callback
# ─────────────────────────────────────────────────────────────────────────────
class SettingsWindow:
    def __init__(self, cfg: dict,
                 ...
                 on_history_click: Optional[Callable[[dict], None]] = None):
        self.cfg = cfg
        ...
        self.win = ctk.CTk()
        self.win.title("AI Mouse-Lens · Elite Designer Settings")
    ...
    def _build(self):
        ...
        self._hdr_hk = ctk.CTkLabel(hdr, text=self.cfg["hotkey"].upper(),
        ...
        sec("SESSION HISTORY")
        self._hist_text = tk.Text(b, height=8, font=(FN, 9), wrap="word",
                                  bg=C["bg"], fg=C["text"],
    ...
    def _on_type_pick(self, choice: str):
        ...
        def sub(t):
            ctk.CTkLabel(frame, text=t, font=ctk.CTkFont(FN, 9),
                         text_color=C["muted"], anchor="w").pack(fill="x", padx=10, pady=(6, 1))
        ...
        if not is_ollama:
            ...
            ctk.CTkEntry(frame, textvariable=self._form_key_var,
                         ...
                         show="•", font=ctk.CTkFont(FN, 11)).pack(fill="x", padx=10)
        if ptype == "custom":
        ...
        self._model_menu = ctk.CTkOptionMenu(model_row,
    ...
    def _do_fetch_voices(self):
        try:
            ...
            self.win.after(0, lambda: self.voice_combo.configure(values=["Error fetching"]))

    # ── History click handler ────────────────────────────────────────────────
    def _on_history_click(self, event):
        """Extract clicked line and notify callback."""
        try:
            ...
            if self.on_history_click:
                # We need the full history item; the line contains a snippet.
                # We'll search the history list by matching the beginning of the snippet.
                items = self.history_fn() if self.history_fn else []
                # reverse to find most recent
                for item in reversed(items):
                    if item["text"].startswith(line[:30]):
...
# ─────────────────────────────────────────────────────────────────────────────
#  APP – master controller, history persistence, bubble control
# ─────────────────────────────────────────────────────────────────────────────
class App:
    def __init__(self):
        self.cfg     = load_cfg()
        ...
        # Overlay will be built after settings window (needs root)
        ...
        self.overlay = Overlay(self.settings.win, self.cfg,
                               on_bubble_result=self._on_bubble_result)
    ...
    # ── history click → re‑open bubble ──────────────────────────────────────
    def _show_history_bubble(self, item: dict):
        """Called when a history line is clicked in Settings."""
        text   = item.get("text", "")
        status = item.get("status", "success")
        # show result and pin it so it stays
        self.overlay.show_result(text, status)
        # force pin
    ...
    # ── bubble result callback (if needed) ──────────────────────────────────
    def _on_bubble_result(self, text, status):
        # Could be used to update history, but we already add separately.
        pass

    def run(self):
        hk  = self.cfg.get("hotkey",       "alt+s").upper()
        vhk = self.cfg.get("voice_hotkey", "alt+v").upper()
        print(f"[MouseLens] Elite ready  ·  {hk} = screen  ·  {vhk} = voice")
        self.settings.run()
