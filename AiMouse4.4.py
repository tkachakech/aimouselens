"""
╔══════════════════════════════════════════════════════════════════╗
║      AI MOUSE-LENS  ·  CONSENSUS EDITION  v4.3                   ║
║  Mode 1: Screen Analysis  ·  Mode 2: Voice Q&A                   ║
║  Mode 3: Contextual Voice Chat  +  Laser Pointer targeting       ║
║  True Silent Failover (401/402/404/429)  ·  Voice Dropdown       ║
║  API Key Auto-Detection  ·  JPEG image optimisation              ║
╚══════════════════════════════════════════════════════════════════╝
...
# ── stdlib ───────────────────────────────────────────────────────────────────
import asyncio, base64, difflib, io, json, math, re, sys
...
PROMPT_CONTEXT_TPL = (
    ...
    "has asked: \"{question}\"\n"
    "Answer their question about what is visible in the image in 1–3 concise "
    "sentences. Be direct and helpful. No preamble.\n"
    "IMPORTANT: If the user is asking WHERE something is located (e.g. 'where "
    "is the button', 'find the search bar', 'point to X'), append its position "
    "on a new line in this exact format: TARGET:[x, y, width, height] where "
    "x/y are the pixel coordinates of the top-left corner of that element "
    "within the captured image and width/height are its size in pixels. "
    "Keep the spoken answer short (1–2 sentences). Do NOT include the TARGET "
    "tag in the spoken part of your answer."
)
PROMPT_OLLAMA = (
...
SMART_PROVIDERS = {
    ...
    "Groq (Fast Voice)": {"type": "custom", "model": "llama-3.3-70b-versatile",
                          "base_url": "https://api.groq.com/openai/v1/chat/completions"},
    "SambaNova (Qwen)": {"type": "custom", "model": "Qwen2.5-72B-Instruct",
...
class AIEngine:
    ...
    @staticmethod
    def _b64(img) -> str:
        """Convert a PIL screenshot to a compact RGB JPEG (≤800px wide,
        quality=75) to minimise token usage and latency."""
        # Convert any mode (RGBA, P, …) to plain RGB first
        rgb = img.convert("RGB")
        # Downscale to max 800px width while preserving aspect ratio
        max_w = 800
        if rgb.width > max_w:
            scale = max_w / rgb.width
            rgb = rgb.resize(
                (max_w, int(rgb.height * scale)),
                resample=3,          # LANCZOS / ANTIALIAS
            )
        buf = io.BytesIO()
    ...
    def _is_soft_error(exc_str: str) -> bool:
        """Return True for 401/402/404/429 — all silently skipped, UI stays in
        Thinking… while the next provider is attempted."""
        return any(code in exc_str for code in ("401", "402", "404", "429"))
...
class Overlay:
    ...
    def _build_bubble(self):
        ...
        # ── Pin button (packed right → sits just left of ✕) ─────────────────
        self._pin_btn = tk.Label(hdr, text=" 📌 ", fg=C["muted"],
                                  bg=C["bg"], cursor="hand2", font=(FN, 10))
        self._pin_btn.pack(side="right")
        self._pin_btn.bind("<Button-1>", lambda _: self._toggle_pin())
        self._pin_btn.bind("<Enter>",    lambda _: self._pin_btn.config(
            fg=C["text"] if not self._pinned else C["accent"]))
        self._pin_btn.bind("<Leave>",    lambda _: self._pin_btn.config(
            fg=C["accent"] if self._pinned else C["muted"]))
    ...
    def hide_bubble(self):
        if self._pinned:
            return  # 📌 pinned – user must click pin again to unpin
        if self._hide_job:
    ...
    def _toggle_pin(self):
        """Toggle result pinning — pinned bubbles never auto-close."""
        self._pinned = not self._pinned
        if self._pinned:
            # Activate: cancel any pending hide job and freeze the timer label
            self._pin_btn.config(fg=C["accent"])
            if self._hide_job:
                self.root.after_cancel(self._hide_job)
                self._hide_job = None
            if self._bubble_vis and self._bw.winfo_exists():
                self._btimer.config(text="📌 pinned")
        else:
            # Deactivate: restore muted colour and restart auto-hide
            self._pin_btn.config(fg=C["muted"])
            if self._bubble_vis and self._bw.winfo_exists():
                self._btimer.config(text=f"auto-close {AUTO_HIDE_SEC}s")
                self._hide_job = self.root.after(
                    AUTO_HIDE_SEC * 1000, self.hide_bubble)
    ...
    # ── Laser-pointer / Teacher-mode target box ───────────────────────────────
    def show_target_box(self, x: int, y: int, w: int, h: int,
                        duration_ms: int = 5000):
        """Draw a neon-cyan pulsing border box at absolute screen coords
        (x, y, w, h).  Disappears automatically after *duration_ms*."""
        BORDER   = 3          # px of the neon outline
        PAD      = 4          # extra padding around the element
        NEON     = "#00fff7"  # neon-cyan
        BG_TRANSP = "#010102"  # near-black used as transparent key
        ...
        win.attributes("-transparentcolor", BG_TRANSP)
        win.configure(bg=BG_TRANSP)
        win.geometry(f"{bw}x{bh}+{bx}+{by}")

        cv = tk.Canvas(win, width=bw, height=bh,
                       bg=BG_TRANSP, highlightthickness=0)
        cv.pack()

        # Draw the neon rectangle (filled transparent, outline only)
        rect_id = cv.create_rectangle(
            BORDER, BORDER, bw - BORDER, bh - BORDER,
            outline=NEON, width=BORDER, fill=BG_TRANSP,
        )
        # Corner accents (L-shaped ticks at each corner)
        tick = min(18, bw // 4, bh // 4)
        corners = [
            (BORDER, BORDER,           BORDER + tick, BORDER),
            (BORDER, BORDER,           BORDER, BORDER + tick),
            (bw - BORDER - tick, BORDER, bw - BORDER, BORDER),
            (bw - BORDER, BORDER,      bw - BORDER, BORDER + tick),
            (BORDER, bh - BORDER,      BORDER + tick, bh - BORDER),
            (BORDER, bh - BORDER - tick, BORDER, bh - BORDER),
            (bw - BORDER - tick, bh - BORDER, bw - BORDER, bh - BORDER),
            (bw - BORDER, bh - BORDER - tick, bw - BORDER, bh - BORDER),
        ]
        for x0, y0, x1, y1 in corners:
            cv.create_line(x0, y0, x1, y1, fill="#ffffff", width=2)

        # Pulse animation — flicker the alpha between 0.6 and 1.0
        _phi = [0.0]
        _job = [None]

        def _pulse():
            if not win.winfo_exists(): return
            _phi[0] = (_phi[0] + 0.18) % (2 * math.pi)
            alpha = 0.65 + 0.35 * math.sin(_phi[0])
            try: win.attributes("-alpha", alpha)
            except: pass
            _job[0] = self.root.after(30, _pulse)

        _pulse()

        def _destroy():
            if _job[0]:
                try: self.root.after_cancel(_job[0])
                except: pass
            try: win.destroy()
            except: pass
...
class SettingsWindow:
    def __init__(self, cfg: dict, on_save,
        ...
        self._hist_line_map: Dict[int, int] = {}   # line_num → items[] index
    ...
    def _build(self):
        ...
        # Grab the Tkinter variable safely on the main thread first
        current_voice = self.tts_var.get()
        threading.Thread(target=self._fetch_voices_bg, args=(current_voice,), daemon=True).start()
        ...
        # ── history ──────────────────────────────────────────────────────────
        sec("SESSION HISTORY")
        hist_frame = tk.Frame(b, bg=C["bg"],
                              highlightbackground=C["border"],
                              highlightthickness=1)
        hist_frame.pack(fill="x", padx=10, pady=(0, 2))
        self._hist_box = tk.Text(
            hist_frame, height=8,
            bg=C["bg"], fg=C["text"], font=(FN, 9),
        ...
        # Colour tags
        self._hist_box.tag_configure("row",     foreground=C["text"])
        self._hist_box.tag_configure("hover",   foreground=C["accent"],
                                                background=C["bg2"])
        self._hist_box.tag_configure("ok_dot",  foreground=C["success"])
        self._hist_box.tag_configure("err_dot", foreground=C["error"])
        self._hist_box.configure(state="disabled")
        # Hint label
        ctk.CTkLabel(b, text="↑ Click any entry to re-display it in the overlay",
                     font=ctk.CTkFont(FN, 8), text_color=C["muted"],
                     anchor="w").pack(fill="x", padx=12, pady=(0, 4))
        # Bindings
        self._hist_box.bind("<Button-1>", self._on_history_click)
        self._hist_box.bind("<Motion>",   self._on_history_hover)
        self._hist_box.bind("<Leave>",    self._on_history_leave)
    ...
    def _poll_history(self):
        """Refresh the clickable history text box every 3 s."""
        if self.history_fn:
            ...
            if items:
                ...
                base   = len(items) - 1          # index of items[-1]
                for i, e in enumerate(recent):
                    line_num = i + 1
                    self._hist_line_map[line_num] = base - i
                    icon     = "✓" if e["status"] == "success" else "✗"
                    dot_tag  = "ok_dot" if e["status"] == "success" else "err_dot"
                    time_str = f"[{e['time']}] "
                    mode_str = f"[{e.get('mode','?')}]  "
                    body_str = e["text"]
                    if i > 0:
                        self._hist_box.insert("end", "\n")
                    self._hist_box.insert("end", time_str, "row")
                    self._hist_box.insert("end", icon + "  ", dot_tag)
                    self._hist_box.insert("end", mode_str, "row")
                    self._hist_box.insert("end", body_str, "row")
            self._hist_box.configure(state="disabled")
        self.win.after(3000, self._poll_history)

    # ── History interaction ───────────────────────────────────────────────────
    def _on_history_click(self, event):
        """Re-display the clicked history entry in the overlay."""
        if not self.history_cb or not self.history_fn:
            return
        line_num = int(self._hist_box.index(
            f"@{event.x},{event.y}").split(".")[0])
        item_idx = self._hist_line_map.get(line_num)
        if item_idx is None:
            return
        items = self.history_fn()
        if 0 <= item_idx < len(items):
            self.history_cb(items[item_idx])

    def _on_history_hover(self, event):
        """Highlight the row under the cursor."""
        self._hist_box.tag_remove("hover", "1.0", "end")
        line_num = int(self._hist_box.index(
            f"@{event.x},{event.y}").split(".")[0])
        if self._hist_line_map.get(line_num) is not None:
            self._hist_box.tag_add("hover",
                                   f"{line_num}.0",
                                   f"{line_num}.end")
...
class AudioPlayer:
    ...
    def _bg(self, text: str, on_done):
        ...
        try:
            ...
            if HAS_PYGAME:
                with self._lock:
                    # ── Strategy 1: load via Sound (avoids bad-ID3-tag rejection) ──
                    played = False
                    try:
                        sound = _pygame.mixer.Sound(tmp)
                        ch = sound.play()
                        if ch:
                            while ch.get_busy():
                                time.sleep(0.05)
                        ...
                        print(f"[TTS] Sound load failed ({e1}), trying music…")

                    # ── Strategy 2: music.load fallback ──────────────────────────
                    if not played:
                        try:
                            ...
                            print(f"[TTS] music fallback also failed ({e2})")
            else:
                # No pygame — wait an estimated duration based on file size
...
class App:
    def __init__(self):
        ...
        self.settings = SettingsWindow(
            ...
            history_cb=lambda item: self._show_history_bubble(item))
    ...
    def _context_worker(self, mx: int, my: int):
        ...
        # Store capture origin so we can translate image-relative coords
        cap_half = self.cfg.get("capture_size", 400) // 2
        cap_origin_x = max(0, mx - cap_half)
        cap_origin_y = max(0, my - cap_half)
        ...
        # ── Parse optional TARGET tag ─────────────────────────────────────────
        target_coords: Optional[Tuple[int, int, int, int]] = None
        _TARGET_RE = re.compile(
            r"TARGET:\s*\[\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\]",
            re.IGNORECASE,
        )
        m = _TARGET_RE.search(result)
        if m:
            rx, ry, rw, rh = (int(m.group(i)) for i in range(1, 5))
            # Translate image-relative coords → absolute screen coords
            target_coords = (
                cap_origin_x + rx,
                cap_origin_y + ry,
                rw, rh,
            )
            # Strip the TARGET tag (and any surrounding whitespace/newlines)
            result = _TARGET_RE.sub("", result).strip()
            print(f"[Mode3] TARGET → screen {target_coords}")

        def ui():
            self.overlay.show_result(result, status, "Mode 3")
            # Laser-pointer box (fires even while TTS is still playing)
            if target_coords and status == "success":
                self.overlay.show_target_box(*target_coords)
            self.overlay.set_state("talking")
            self.audio.speak(result, on_done=lambda:
                self.settings.win.after(
                    0, lambda: self.overlay.set_state("idle")))
    ...
    def _show_history_bubble(self, item: dict):
        """Re-display a history item in the overlay and auto-pin it."""
        text   = item.get("text", "")
        status = item.get("status", "success")
        mode   = item.get("mode", "History")
        # Temporarily clear pin so show_result runs its normal path,
        # then engage _toggle_pin to freeze it on screen.
        self.overlay._pinned = False
        self.overlay.show_result(text, status, f"📌 {mode}")
        if not self.overlay._pinned:
            self.overlay._toggle_pin()
